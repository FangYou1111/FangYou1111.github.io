            ...
            <input
              ...
              @change="
                handler(
                  'change',
                  cart,
                  parseInt($event.target.value) * 1 - cart.skuNum
                )
              "
            />
            <a
...
export default {
  ...
  computed: {
    ...
    totalPrice() {
      ...
      if (this.cartInfoList) {
        this.cartInfoList.forEach((item) => {
          if (item.isChecked === 1) {
            result += item.skuNum.toFixed(2) * item.skuPrice.toFixed(2);
          }
        });
      }
      return result;
    ...
    isAllChecked() {
      let re = true;
      if (this.cartInfoList) {
        re = this.cartInfoList.every((item) => {
  ...
  methods: {
    ...
    handler: throttle(async function (type, cart, changeNumber) {
      try {
        if (type === "change" && isNaN(changeNumber)) {
          changeNumber = 0;
        }
        if (changeNumber < 0 && cart.skuNum < Math.abs(changeNumber)) {
          changeNumber = 0;
        }
    ...
    async changeAllChecked(event) {
      try {
        ...
        await this.$store.dispatch("shopCart/changeAllCheckedCart", checked);
        this.getData();
      } catch (error) {
        alert(error.message);
      }
